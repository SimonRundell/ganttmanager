 ...
 */
// Allow credentials for httpOnly cookie auth in dev
...
// Auth is enforced per-endpoint for this app.
...
function require_auth($config, $mysqli, $roles = []) {
    ...
    $stmt = $mysqli->prepare('SELECT id, email, screen_name, avatar_path, role, is_verified FROM users WHERE id = ? LIMIT 1');
    if (!$stmt) {
...
function get_bearer_token() {
    ...
    if (isset($_COOKIE['gm_token'])) {
        return $_COOKIE['gm_token'];
    }
...
function set_auth_cookie($token, $expiresAt) {
    $expires = is_numeric($expiresAt) ? (int) $expiresAt : time() + 3600;
    setcookie('gm_token', $token, [
        'expires' => $expires,
        'path' => '/',
        'secure' => false,
        'httponly' => true,
...
function create_jwt($userId, $config, $role = null) {
    $secret = $config['jwtSecret'] ?? '';
    if ($secret === '') {
